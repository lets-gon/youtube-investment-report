...
GitHub Actions가 매일 YouTube 채널 8개의 최근 24시간 콘텐츠를 확인하고, 가능한 경우 YouTube 자막/자동자막을 먼저 수집합니다. 자막이 없으면 영상 오디오를 전사한 뒤 OpenAI API로 한국어 보고서를 만들고 텔레그램으로 보냅니다.
...
3. `음성 전사 기반`: 자막이 없으면 영상 오디오를 전사해서 요약합니다.
4. `설명란 기반`: 자막과 음성 전사가 모두 실패했을 때 영상 설명란을 바탕으로 요약합니다.
5. `제목만 확인`: 자막, 음성 전사, 설명란이 모두 부족할 때 제목만 확인된 항목으로 표시합니다.
6. `게시물 원문 기반`: `asset.x2` 커뮤니티 게시물처럼 영상이 아닌 글 원문을 요약합니다.

텔레그램에는 영상별로 `요약 근거`, `영상 내용 요약`, `핵심 포인트`, `투자자 체크`가 표시됩니다. Google Drive HTML 보고서와 SQLite DB에는 자막/자동자막/음성 전사 일부와 상세 요약이 함께 저장됩니다.
...
3. 필요하면 `Settings → Secrets and variables → Actions → Variables`에 `OPENAI_MODEL`을 추가합니다.
   ...
   - `MAX_TRANSCRIPT_CHARS`: DB에 저장할 영상별 자막/전사문 최대 길이입니다. 기본값은 `12000`입니다.
   - `PROMPT_TRANSCRIPT_CHARS`: OpenAI 요약에 보낼 영상별 자막/전사문 최대 길이입니다. 기본값은 `5000`입니다.
   - `TRANSCRIPT_LANGUAGES`: 우선 수집할 자막 언어입니다. 기본값은 `ko,en`입니다.
   - `ENABLE_AUDIO_TRANSCRIPTION`: 자막이 없을 때 음성 전사를 사용할지 정합니다. 기본값은 `1`입니다.
   - `OPENAI_TRANSCRIBE_MODEL`: 음성 전사 모델입니다. 기본값은 `whisper-1`입니다.
   - `MAX_AUDIO_FILE_MB`: 전사용 오디오 파일 최대 크기입니다. 기본값은 `24`입니다.
...
- `raw`: YouTube RSS, 자막/자동자막, 음성 전사, 커뮤니티 게시물 원본 수집 데이터입니다.
...
```bash
DRY_RUN=1 python audio_report.py
```
